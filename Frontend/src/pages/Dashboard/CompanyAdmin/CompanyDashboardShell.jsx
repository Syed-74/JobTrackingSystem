...
const CompanyDashboardShell = ({ allowedTabs = [], roleTitle = 'COMPANY USER' }) => {
  ...
  const [activeTab, setActiveTab] = useState(
    allowedTabs.includes('overview') ? 'overview' : allowedTabs[0]
  );
  const [settingsSubTab, setSettingsSubTab] = useState(
    (user?.role === 'COMPANY_ADMIN' || user?.role === 'HR Manager') ? 'profile' : 'security'
  );
  ...
  const initDashboard = async () => {
    ...
    try {
      ...
      try {
        const dbDepts = await getAllDepartments(companyId);
      ...
      // Fetch company staff accounts
      try {
        const dbAccounts = await getAllCompanyAccountManage(companyId);
        setCompanyAccounts(dbAccounts || []);
      } catch (err) {
      ...
      try {
        const dbJobs = await getAllJobPostings();
        setJobs(dbJobs || []);
      } catch (err) {
        console.error("Failed to load job postings from database", err);
        setJobs([]);
      }
  ...
  const isAllAccess = user?.role === 'COMPANY_ADMIN' || user?.role === 'HR Manager';

  const visibleJobs = isAllAccess 
    ? jobs 
    : jobs.filter(j => j.assignedUserId === user?.id || j.assignedUserId === user?.admin_id);

  const visibleApplications = isAllAccess
    ? applications
    : applications.filter(app => {
        const job = jobs.find(j => j.title === app.jobTitle);
        return job && (job.assignedUserId === user?.id || job.assignedUserId === user?.admin_id);
      });

  // Dynamically calculate stats based on visible state
  const dashboardStats = {
    departments: departments.length,
    employees: employees.length,
    activeJobs: visibleJobs.filter(j => j.status === 'Active').length,
    applications: visibleApplications.length,
    selected: visibleApplications.filter(a => a.status === 'Selected').length
  };

  return (
    <div className="min-h-screen bg-neutral-base font-sans relative flex w-full">
      ...
      <div className="flex-1 flex flex-col min-w-0 z-10 relative w-full">
            ...
            <div className="relative" ref={profileRef}>
              ...
              >
                ...
                <div className="hidden md:flex flex-col items-start text-left">
                  ...
                  <span className="text-[10px] text-neutral-text-muted">
                    {user?.role === 'COMPANY_ADMIN' ? 'Workspace Owner' : user?.role || 'Staff Member'}
                  </span>
                </div>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-2.5 w-[220px] bg-neutral-surface border border-neutral-border rounded-theme-xl shadow-theme-lg py-2 z-50 animate-in fade-in slide-in-from-top-3 duration-200">
                  ...
                  <div className="py-1">
                    {(user?.role === 'COMPANY_ADMIN' || user?.role === 'HR Manager') && (
                      <button 
                        onClick={() => { setIsProfileOpen(false); setActiveTab('settings'); setSettingsSubTab('profile'); }}
                    ...
                    <button 
                      onClick={() => { setIsProfileOpen(false); setActiveTab('settings'); setSettingsSubTab('security'); }}
                      className="w-full text-left px-4 py-2 text-xs text-neutral-text hover:bg-neutral-muted flex items-center gap-2.5 transition-colors"
                    >
        ...
        {/* VIEW BODY CONTENT */}
        <main className="flex-1 p-4 md:p-6 lg:p-8 overflow-y-auto z-10 relative w-full">
          ...
          {loading ? (
            ...
            <>
              ...
              {activeTab === 'departments' && allowedTabs.includes('departments') && (
                <DepartmentsTab
              ...
              {activeTab === 'jobs' && allowedTabs.includes('jobs') && (
                <JobPostsTab
                  jobs={visibleJobs}
                  departments={departments}
              ...
              {activeTab === 'applications' && allowedTabs.includes('applications') && (
                <JobApplicationsTab
